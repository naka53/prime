...
Dans notre démarche de recherche de la syscall table, nous nous sommes demandés s'il n'était pas possible de retrouver la syscall table en attaquant la mémoire noyau directement. Pour cela nous avons dû bien comprendre le fonctionnement de celle-ci, et voir comment on pourrait tenter de détourner de façon malveillante l'API.
\\
Dans les systèmes actuels, la mémoire est différenciée en deux types, nous avons d'un coté la mémoire dite virtuelle et de l'autre la mémoire dite physique. La mémoire virtuelle est utilisée au sein des processus pour faire croire à ceux-ci qu'ils disposent d'un grand espace de mémoire contiguë, la mémoire physique quant à elle est celle réellement installée sur le système (RAM,..). Lorsqu'un processus demande l'accès à une adresse particulière, le CPU ne connaît pas les translations, il va donc demander à un autre composant cette translation,le MMU.
\\
\includegraphics[scale=0.3]{./mmu.png}
\\
Cependant la translation est longue car le MMU parcourt toutes les tables jusqu'à arriver à celle qu'il l'intéresse et donc afin de rendre la translation plus rapide et plus efficace, le MMU dispose d'un cache particulier appelé TLB (translation lookaside buffer).
\\
Voici un bref schéma de fonctionnement du MMU
\includegraphics[scale=0.7]{./TLB.png}
Il faut savoir que le système travaille avec des pages mémoire de 4KB, il dispose aussi d'une manière très particulière de gérer ces tables. En effet, une page particulière appartient à une Page Table, qui elle même est contenue dans un Page directory. Chaque processus dispose de son propre Page directory et les processeurs actuels utilisent le registre CR3 pour savoir lequel utiliser.

\section{Une première Attaque (ATRA)}
Lors de nos recherche, nous n'avons pas trouver de méthode efficace pour fouiller la mémoire noyau à la recherche de la syscall table. 
Cependant nous avons pu trouver une méthode qui consiste, une fois la syscall table trouvée, la dupliquer en mémoire afin de faire utiliser notre copie par le système. Cette méthode s'appelle ATRA  (Adress Translation Redirection Attack), cette attaque consiste à récupérer l'adresse de la syscall table, de créer une nouvelle page mémoire et de copier le contenu de la syscall table dans celle-ci. Pour se faire, on doit retrouver l'adresse physique de la page qui contient la syscall table, et copier son contenu dans la nouvelle. Ensuite, et là réside la difficulté, il nous faut modifier l'entrée correspondante à la vrais syscall table dans la page table en la faisant pointer vers l'adresse physique de notre copie.
\\
Cette attaque est surtout employée pour rendre un rootkit plus "furtif" dans le cas où il y a des sécurités hardware sur le système, en effet en faisant une copie et en ne modifiant pas la vrais syscall table le moniteur qui est chargé de vérifier sa validité ne verra pas les modifications.
...
Dans cette partie, nous allons discuter d'une idée potentielle pour trouver l'adresse de la syscall table au sein du système. En regardant de plus près le fonctionnement de la mémoire, nous pouvons voir que les pages noyau (contenant les structures) sont toutes partagées entre chaque processus. C'est à dire que dans chaque espace mémoire de chaque processus on peut retrouver un pointeur vers les structures noyau. On peut retrouver le registre CR3 avec un code assembleur. Donc il pourrait en pratique être possible d'explorer le PGD d'un processus et de retrouver dans sa mémoire un pointeur vers la syscall table. Cependant nous nous sommes heurtés à une sécurité (PTI) qui nous empêchée, même avec les droits noyau, d'aller explorer des pages mémoire en dehors de notre module. 
\section{Conclusion}
...
\\
Nous pouvons aussi voir qu'il existe des failles au sein de la mémoire elle même. Cependant elles restent compliquées à exploiter aux vues de la difficulté et du peu de documentation qu'il y a sur les codes linux gérant la mémoire.
