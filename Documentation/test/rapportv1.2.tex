...
\section{Fonctionnement Memoire}
Dans notre démarche de recherche de la syscall table, nous nous sommes demandés s'il n'était pas possible de retrouver la syscall table en attaquant la mémoire kernel directement. Pour cela nous avons tout du tout d'abord bien comprendre le fonctionnement de celle-ci, et voir comment on pouvait tenter de détourner de façon malveillante l'API.
\\
Dans les systèmes actuels, la mémoire est différenciée en deux types, nous avons d'un coté la mémoire dites virtuelle et de l'autre la mémoire dites physique. La mémoire virtuelle est utilisée au sein des processus pour faire croire à celui-ci qu'il dispose d'un grand espace de mémoire contiguë, la mémoire physique quant à elle est celle réellement installé sur le système (RAM,..). Lorsqu'un processus demande L?accès à une adresse particulière, le CPU lui ne connaît pas les translations, il va donc demander à un autre composant cette translation, ce composant est le MMU.
\\
\includegraphics[scale=0.3]{./mmu.png}
\\
Il faut savoir que le système travaille avec des pages mémoires de 4KB (généralement), il dispose aussi d'une manière très particulière de de gérer ces tables. En effet, une page particulière appartient à une Page Table, qui lui même est contenue dans un Page directory. Chaque processus dispose de son propre Page directory et les processeurs actuels utilisent le registre CR3 pour savoir dans quel contexte ils sont. De plus, afin de rendre la translation plus rapide et plus efficace, car le parcourt de l'arborescence des tables peut s'avérer long, le MMU dispose d'un cache particulier appelé TLB (translation lookaside buffer).
\\
Voici un bref schéma de fonctionnement du MMU
\includegraphics[scale=0.7]{./TLB.png}
\section{Une première Attaque (ATRA)}
Lors de nos recherche, nous n'avons pas trouver de méthode efficace pour fouiller la mémoire Kernel à la recherche de la syscall table, cependant nous avons pu trouver une méthode qui consiste à une fois la syscall table trouvée, la dupliquer en mémoire afin de faire utiliser notre copie par le système. Cette méthode s'appelle ATRA  Adress Translation Redirection Attack), cette attaque consiste à récupérer l'adresse de la syscall table, de créer une nouvelle page mémoire et de copier le contenue de la syscall table dans celle-ci. Pour ce faire, on doit tout d'abord retrouver l'adresse physique de la page qui contient la syscall table, et copier son contenu dans la nouvelle. Ensuite, et là réside la difficulté, il nous faut modifier l'entrée correspondant à la vrais syscall table dans la page table en la faisant pointer vers l'adresse physique de notre copie.
\\
Cette attaque est surtout employé pour rendre un rootkit plus "furtif" dans le cas où il y a des sécurité hardware sur le système, en effet en faisant une copie et en ne modifiant pas la vrais  syscall table le moniteur qui est chargé de vérifier sa validité ne verra pas les modifications.
\includegraphics[scale=0.22]{./ATRA.png}
\begin{center}
schéma de la mémoire après ATRA.
\end{center}
On peut aussi noter que ATRA peut être faites en modifiant la valeur des PGD, cependant cela ne rend pas l'attaque complète car comme dit plus haut chaque processus a son propre PGD, il faudrait donc faire la modification pour tous les processus.
\section{Une seconde attaque}
Dans cette partie, nous allons discuter d'une idée potentielle pour trouver l'adresse de la syscall table au sein du système. En regardant de plus près le fonctionnement de la mémoire, nous pouvons voir que les pages kernel (contenant les structures) sont toutes partagées entre chaque processus. C'est à dire que dans chaque espace mémoire de chaque processus on peut retrouver un pointeur vers les structures kernel. De plus on peut retrouver le registre CR3 avec un code assembleur, ainsi il pourrait en pratique être possible d?explorer le PGD d'un processus et de retrouver dans sa mémoire un pointeur vers la syscall table, cependant nous nous sommes heurtés à une sécurité (PTI), qui nous empêcher, même avec les droit kernel, d'aller explorer des pages mémoire en dehors de notre module. 
\section{Conclusion}
...
\\
Nous pouvons aussi voir qu'il existe aussi des failles au sein de la mémoire elle même, mais qu'elles restent relativement compliquer à mettre en place,au vue de la difficulté et du peu de documentation qu'il y a sur le code linux qui gère la mémoire.
